...
module.exports = () => {
    ...
    */
    ...
    route.post('/postEmployeeCertifications', (req, res) => {
        let mObj = {}
        for (let obj in req.body) {
          mObj = JSON.parse(obj)
        }
        ...
        let name = editedItem.name
        let frigana = editedItem.frigana
        let enteringDate = editedItem.enteringDate
        let certificationName = editedItem.certificationName
        let certificationGetDate = editedItem.certificationGetDate
        let bonusGetDate = editedItem.bonusGetDate

        let selSql = `SELECT e.employee_id ,name,frigana ,DATE_FORMAT( entering_date , '%Y/%m/%d' ) entering_date,
        certification_name, DATE_FORMAT( get_date , '%Y/%m/%d' ) get_date,  DATE_FORMAT( encourage_date , '%Y/%m/%d' ) encourage_date
        from employee e,employee_certification c where e.employee_id = c.employee_id `;

        if (employeeId && employeeId != ""){
            selSql = `${selSql} AND e.employee_id = '${employeeId}'`;
        }
        if (name && name != ""){
            selSql = `${selSql} AND e.name = '${name}'`;
        }
        if (frigana && frigana != ""){
            selSql = `${selSql} AND e.frigana = '${frigana}'`;
        }
        if (enteringDate && enteringDate != ""){
            selSql = `${selSql} AND e.entering_date = '${enteringDate}'`;
        }
        if (certificationName && certificationName != ""){
            selSql = `${selSql} AND c.certification_name = '${certificationName}'`;
        }
        if (certificationGetDate && certificationGetDate != ""){
            selSql = `${selSql} AND c.get_date = '${certificationGetDate}'`;
        }
        if (bonusGetDate && bonusGetDate != ""){
            selSql = `${selSql} AND c.encourage_date = '${bonusGetDate}'`;
        }
    ...
    route.post('/saveEmployeeCertification', (req, res) => {
        ...
        let get_date = editedItem.certificationGetDate
        let encourage_date = editedItem.bonusGetDate
        ...
        // db.query(insEmployee_certification, (err, data) => {
        //     if (err) {
        //         console.log(err);
        //         res.status(500).send('database err').end();
        //     } else {
